...
import com.example.BTL.jwt.JwtService;
import com.example.BTL.token.Token;
import com.example.BTL.token.TokenRepository;
...
public class AuthenticationService {
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest request){
        ...
        if(user == null){
            return AuthenticationResponse.builder().message("Username not found").build();
        }
        if( !passwordEncoder.matches(request.getPassword(), user.getPassword())){
            ...
            if (timeLock >= 0){
                return AuthenticationResponse.builder().message("Tài khoản của bạn đang bị khóa hãy thử lại sau " + parseLockTime(timeLock) ).build();
            }
            else {
                ...
                userRepository.save(user);
                return AuthenticationResponse.builder().message("Bạn đã đăng nhập sai " + 1 + " lần. Nếu sai quá 5 lần tài khoản của bạn sẽ bị khóa").build();
            }
        }
        long timeLock = user.getTimeLock().getTime() - System.currentTimeMillis();
        if(timeLock >= 0){
